...
6. 用神：**用户自选**（5 个六亲）+「问什么取什么」口诀提示；选中后只在**本卦**定位高亮——不上卦则取**伏神**；用神多现暂不挑选（待旺衰那期按规则取一）；变卦盘不染用神。
...
7. **用神选择器**：用户自选 5 个六亲之一 + 口诀提示；用神只在**本卦**定位高亮——上卦取诸现爻（多现暂不挑选，待旺衰那期按规则取一），不上卦取**伏神**（高亮其所挂本卦爻、标「用神·伏」）；变卦盘不染用神。
